...
type ShowOverlayParameter struct {
...
func ShowOverlay(showParam *ShowOverlayParameter) (string, error) {
	overlayName := showParam.OverlayName
	fileName := showParam.FilePath
	NodeName := showParam.NodeName
	...
	if NodeName == "" {
		...
		// stringBuffer is used to return rendered content
		...
		for bufferScanner.Scan() {
			...
			if len(filenameFromTemplate) != 0 {
				...
				if foundFileComment {
					...
					stringBuffer.WriteString(outBuffer.String())
					outBuffer.Reset()
				}
